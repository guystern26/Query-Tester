...
export function Modal({ open, title, children, onClose, confirmLabel, onConfirm, variant = 'default' }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center" onClick={onClose} role="dialog" aria-modal="true">
      <div className="bg-slate-900 rounded-xl border border-slate-700 p-0 w-full max-w-md shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-6 py-3 border-t border-slate-700 flex justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 text-sm font-medium bg-slate-800 border border-slate-600 text-slate-300 hover:border-slate-500 rounded-lg transition cursor-pointer"
          >
            Cancel
          </button>
          {confirmLabel != null && onConfirm != null && (
            <button
              type="button"
              onClick={onConfirm}
              className={`px-4 py-2 text-sm font-semibold rounded-lg transition cursor-pointer ${confirmCls}`}
            >
              {confirmLabel}
            </button>
          )}
