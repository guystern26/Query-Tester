...
export const vSelectCls =
  'bg-slate-950 border border-slate-800 rounded text-xs text-slate-300 px-1.5 py-1 cursor-pointer focus:outline-none focus:border-cyan-500';
...
export function VRow({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex gap-1.5 items-center bg-slate-950 border border-slate-800 rounded-lg px-2 py-1.5 mb-1">
      {children}
    </div>
  );
}
...
export function VAddBtn({ onClick, disabled, label }: { onClick: () => void; disabled?: boolean; label: string }) {
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="flex items-center gap-1 text-xs text-slate-400 hover:text-cyan-400 mt-1 border-none bg-transparent cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
    >
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
