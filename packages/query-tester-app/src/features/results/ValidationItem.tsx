...
export function ValidationItem({ v, fieldExistsInResults }: ValidationItemProps) {
  ...
  const isMessageOnly = !v.field && !v.condition;
  ...
  return (
    <div className={`flex items-start gap-2 px-3 py-2 rounded ${
      ...
      <div className="flex flex-col gap-0.5 min-w-0">
        {isMessageOnly ? (
          <div className={`text-[13px] ${v.passed ? 'text-green-300/80' : 'text-red-300/80'}`}>
            {v.message}
          </div>
        ) : (
          <>
            ...
            {!fieldExistsInResults && !v.field.startsWith('_') && v.field && (
