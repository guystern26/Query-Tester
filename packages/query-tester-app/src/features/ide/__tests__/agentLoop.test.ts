...
function makeRestExec(): (path: string, params?: Record<string, string>) => Promise<{ entries: Array<{ name: string; id: string; content: Record<string, unknown> }>; totalCount: number; path: string }> {
    return jest.fn().mockResolvedValue({ entries: [], totalCount: 0, path: '' });
}
...
describe('runAgentPipeline', () => {
    ...
    it('routes through manager → explainer → validator (happy path)', async () => {
        ...
        const result = await runAgentPipeline(
            ...
            makeContext(), [], makeExec(), makeRestExec(),
