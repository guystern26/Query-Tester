 ...
 * Manager → Specialist → (auto-query / splunk_rest loop) → Validator → response.
 */
...
import type { SplunkRestResponse } from '../../api/splunkRestApi';
...
export interface AgentStep {
    id: string;
    type: 'auto_query' | 'debug_step' | 'validation' | 'routing' | 'splunk_rest';
    spl?: string;
...
const AUTO_EXEC_TYPES = ['auto_query', 'splunk_rest', 'debug_pipeline'];
...
function fmtRestEntries(entries: SplunkRestResponse['entries'], total: number): string {
    if (entries.length === 0) return '(0 entries)';
    const lines = entries.slice(0, 10).map((e) => {
        const fields: string[] = ['**' + e.name + '**'];
        if (e.content) {
            const keys = Object.keys(e.content).slice(0, 8);
            for (const k of keys) {
                const v = String(e.content[k] ?? '').slice(0, 100);
                fields.push(k + ': ' + v);
            }
        }
        return '- ' + fields.join(' | ');
...
function parseSplunkRestPayload(payload: string): { path: string; params: Record<string, string> } {
    try {
        ...
        // Fallback: treat as plain path string
        return { path: payload.trim(), params: {} };
    }
}
...
async function executeAutoActions(
    actions: ParsedAction[], spl: string, iterations: number,
    executeQuery: (q: string) => Promise<IdeRunResponse>,
    executeSplunkRest: (path: string, params?: Record<string, string>) => Promise<SplunkRestResponse>,
    onStep: (step: AgentStep) => void, signal: AbortSignal, steps: AgentStep[],
    hasDebug: boolean,
): Promise<string[]> {
    const results: string[] = [];

    // Execute auto_query actions
    const queries = actions.filter((a) => a.type === 'auto_query');
    ...
    // Execute splunk_rest actions
    const restCalls = actions.filter((a) => a.type === 'splunk_rest');
    for (const rc of restCalls) {
        checkAbort(signal);
        const { path, params } = parseSplunkRestPayload(rc.payload);
        const step: AgentStep = {
            id: 'rest-' + iterations + '-' + steps.length, type: 'splunk_rest',
            status: 'running', label: 'REST: ' + path.slice(0, 60),
        };
        steps.push(step);
        onStep(step);
        try {
            const resp = await executeSplunkRest(path, params);
            const upd = {
            ...
            steps[steps.length - 1] = upd;
            onStep(upd);
            results.push('REST `' + path + '`:\n' + fmtRestEntries(resp.entries, resp.totalCount));
        } catch (e) {
            const err = e as { message?: string };
            const upd = { ...step, status: 'error' as const, error: err.message };
            steps[steps.length - 1] = upd;
...
async function runSpecialist(
    ...
    execRest: (path: string, params?: Record<string, string>) => Promise<SplunkRestResponse>,
    ...
    for (let i = 0; i < MAX_AGENT_ITERATIONS; i++) {
        ...
        const autoActions = actions.filter((a) => AUTO_EXEC_TYPES.includes(a.type));
        const hasDebug = actions.some((a) => a.type === 'debug_pipeline');
        if (autoActions.length === 0 && !hasDebug) {
        ...
        const results = await executeAutoActions(
            autoActions, spl, i + 1, exec, execRest, onStep, signal, steps, hasDebug,
        );
...
export async function runAgentPipeline(
    ...
    for (let retry = 0; retry <= MAX_VALIDATOR_RETRIES; retry++) {
        ...
        const result = await runSpecialist(
            specialistCfg, spl, app, timeRange, contextData, history,
            executeQuery, executeSplunkRest, onStep, signal, steps, feedback,
        );
