...
import { callSplunkRest } from '../../../api/splunkRestApi';
    ...
    return async (text: string) => {
        ...
        try {
            ...
            if (agentConfig) {
                ...
                const execRest = (path: string, params?: Record<string, string>) => callSplunkRest(path, params);
                const r = await runAgentPipeline(agentConfig, text, spl, app, timeRange, ctx, history, exec, execRest,
                    (step) => {
                        set((d) => {
